import { gql, request } from 'graphql-request'

const MASTER_URL = "https://ap-south-1.cdn.hygraph.com/content/cm42rwaai01tx07w33vwfkoc5/master"

// courseList
export const getCourseList = async (courseLevel) => {
  const query = gql`
    query CourseList {
        courses(where: {courseLevel: `+ courseLevel + `}) {
            id
            name
            price
            rating
            time
            courseLevel
            author
            description {
                markdown
            }
            banner {
                url
            }
            chapters {
                id
                title
                content {
                    title
                    description {
                        markdown
                        html
                    }
                    output {
                        markdown
                        html
                    }
                }
            }
        }
    }
    `

  const result = await request(MASTER_URL, query);
  return result;
}

// entrollCourse
export const entrollCourse = async (courseId, userEmail) => {
  const mutationQuery = gql`
  mutation entrollCourse {
    createUserEntrolledCourse(
      data: {courseId: "`+ courseId + `", 
              userEmail: "`+ userEmail + `", 
              course: {connect: {id: "`+ courseId + `"}}}
    ) {
      id
    }
    publishManyUserEntrolledCoursesConnection(to: PUBLISHED) {
      edges {
        node {
          id
        }
      }
    }
  }
  `

  const result = await request(MASTER_URL, mutationQuery);
  return result;
}


// user details
export const getUserEntrolledCourse = async (courseId, userEmail) => {
  const query = gql`
  query MyQuery {
  userEntrolledCourses(
    where: {courseId: "`+ courseId + `", 
    userEmail: "`+ userEmail + `"}
  ) {
    id
    courseId
    completedChapter {
      chapterId
    }
  }
}
  `

  const result = await request(MASTER_URL, query);
  return result;
}


// completed Chapter
export const MarkChapterCompleted = async (chapterId, recordId, userEmail, points) => {
  const mutationQuery = gql`
  mutation markChapterCompleted {
  updateUserEntrolledCourse(
    data: {completedChapter: {create: {data: {chapterId: "`+ chapterId + `"}}}}
    where: {id: "`+ recordId + `"}
  ) {
    id
  }
  publishManyUserEntrolledCoursesConnection {
    edges {
      node {
        id
      }
    }
  }

  updateUserDetail(where: {email: "`+ userEmail + `"},
  data: {point: `+ points + `}){
    point
  }
  publishUserDetail(where: {email: "`+ userEmail + `"}) {
    id
  }

}
  `

  const result = await request(MASTER_URL, mutationQuery);
  return result;
}


// create new user
export const createNewUser = async (userName, email, profileImageUrl) => {
  const mutationQuery = gql`
  mutation CreateNewUser {
  upsertUserDetail(
    upsert: {create: {
      email: "`+ email + `", 
      point: 10, 
      profileImage: "`+ profileImageUrl + `", 
      userName: "`+ userName + `"}, 
      update: {email: "`+ email + `", 
        profileImage: "`+ profileImageUrl + `", 
        userName: "abc"}}
    where: {email: "`+ email + `"}
  ) {
    id
  }
  publishUserDetail(where: {email: "`+ email + `"}) {
    id
  }
}
  `

  const result = await request(MASTER_URL, mutationQuery);
  return result;
}


// get user detail
export const getUserDetail = async (email) => {
  const query = gql`
  query getUserDetails {
    userDetail(where:
      {email: "`+ email + `"}) {
      point
    }
  }
  `

  const result = await request(MASTER_URL, query);
  return result;
}


// export const GetAllUsers = async() => {
//   const query = gql`
//     query GetAllUsers {
//       userDetails(orderBy: profileImage_DESC) {
//         id
//         profileImage
//         userName
//         point
//       }
//     }
//   `
//   const result = await request(MASTER_URL, query);
//     return result;
// }



export const GetAllUsers = async () => {
  const query = gql`
    query GetAllUsers {
  userDetails(orderBy: point_DESC) {
    id
    userName
    profileImage {
      url
    }
    profileImage2
    point
  }
}
  `
  const result = await request(MASTER_URL, query);
  return result;
}


export const GetAllProgressCourse = async(userEmail) =>{
  const query = gql`
  query GetAllUserEnrolledProgressCourse {
  userEntrolledCourses(where: {userEmail: "`+userEmail+`"}) {
    completedChapter {
      chapterId
    }
    course {
      banner {
        url
      }
      chapters {
        id
        title
        content {
          heading
          description {
            markdown
            html
          }
          output {
            markdown
            html
          }
        }
      }
      description {
        markdown
      }
      id
      courseLevel
      price
      time
    }
  }
}
  `
  const result = await request(MASTER_URL, query);
  return result;
}

